...
if [[ $numb_of_files == 1 ]]; then # IF SE READS
    ...
    bedtools bamtobed -i ${out}/${experiment}.q${quality}.bam | awk '$6 == "-"' | awk '{OFS="\t";print $4,$1,$3-1,"-"}' > $aux/reverse & pid2=$! # if - strand DSB location is the third field
    wait $pid1
    wait $pid2
...
if [[ $numb_of_files == 2 ]]; then # IF PE READS
    ...
    bedtools bamtobed -i ${out}/${experiment}.q${quality}.bam | awk '$6 == "-"' | awk '{OFS="\t";print $4,$1,$3-1,"-"}' | grep -v '\\2' | sed 's/\/1//' > $aux/reverse & pid2=$!
    wait $pid1
    wait $pid2
    cat $aux/forward $aux/reverse |LC_ALL=C sort --parallel=8 --temporary-directory=$HOME/tmp -k1,1 > $aux/id.chr.loc.strand & pid1=$!
    cat $out/filtered.r1.fa | tr -d ">" | cut -d':' -f-7 | paste - - > $aux/id.umi.barcode.genomic_aux & pid2=$!
